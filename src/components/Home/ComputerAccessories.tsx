            ...
            <div className="flex justify-between items-center h-auto flex-wrap">
              ...
              <div className="flex gap-6 text-sm flex-wrap">
                <ul className="text-gray-600 list-none flex items-center gap-4 flex-wrap">
                  <li className=" py-2 border-b-2 border-b-primary-500 text-gray-900 font-bold  transition-all duration-100 px-2">
                    All Product
                  </li>
                  {
                    brandList !== [] &&
                      brandList.slice(0, 4).map((brand) => (
                        <Link to='/products' key={brand?._id} className=" py-2 hover:text-gray-900 hover:border-b-2 hover:border-b-primary-500 hover:font-bold px-2 transition-all duration-100">
                          {brand?.name}
                        </Link>
                    ))
                  }
          ...
          <div className="2xl:col-span-3 w-full flex flex-col gap-4">
            <div className="bg-warning-200 rounded-lg p-6">
              ...
              <div className="flex gap-4 items-center flex-wrap w-fit mx-auto mb-4">
                ...
                <button className="text-xs bg-gray-00 text-gray-900 font-bold rounded-sm px-4 py-2">
                  {product?.price} ugx
                </button>
              </div>
              <ShopNowButton link={`/product/${product?._id}`} className="w-full" />
            </div>
            {/**/}
            <div className="rounded-lg p-6 bg-dark-blue-800 flex flex-col justify-center">
              <Button
                title="SUMMER SALES"
                className="mt-4 bg-dark-blue-200 text-sm text-gray-00 mx-auto"
              />
              <h3 className="text-gray-00 text-2xl my-4 w-fit mx-auto font-bold">
                37% DISCOUNT
              </h3>
              <p className="text-gray-00 mx-auto mb-4">
                only for <span className="text-warning-500">SmartPhone</span>
                product.
              </p>
